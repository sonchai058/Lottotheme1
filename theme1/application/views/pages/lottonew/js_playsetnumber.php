...
$(".btn_pressactive").click(function(){ //กดเอง & Chanel Click
  if($(this).hasClass("active")) {
    ...
    //alert($(this).data('numlabel').charAt(0));
  ...
  for(a=0;a<numpaddButtonSelected.length;a++) {//Set Item Selected
    ...
    */
    for(k=0;k<jMax;k++){
      ...
      $('<div data-number="'+number+'"  class="deprive'+deprive+' ll'+(itemSelected.length-1)+' listSelected list'+number+'"><div class="content"><span class="txt-theme">'+number.substr(0,3)+'</span><br/><span class="label" style="font-size:15px">('+numtext+')</span></div><div style="font-size:15px !important;position : absolute !important;top:0px !important;right:5px !important;" class="close1"><span class="txt-theme"><i onclick="rmList(this,\''+numlabel+'\','+number+','+(itemSelected.length-1)+')" style="cursor:pointer; color:#e44444" class="fa fa-times" aria-hidden="true"></i></span></div></div>').appendTo('.contentList');
    }
...
$(".btn_cleardata").click(function(){//Clear Data
  ...
  $.ajax({
    type: "POST",
    url: '<?php echo base_url('service/active/cls_listsetnum/'.$this->uri->segment(3)); ?>',
...
function genList2(numlabel,numtext,numberArr) {
  ...
  for(k=0;k<jMax;k++){
      ...
      //console.log(itemSelected);
      $('<div data-number="'+number+'" class="deprive'+deprive+' ll'+(itemSelected.length-1)+' listSelected list'+number+'"><div class="content"><span class="txt-theme">'+number.substr(0,3)+'</span><br/><span class="label" style="font-size:11px">('+numtext+')</span></div><div style="font-size:15px !important;position : absolute !important;top:0px !important;right:5px !important;" class="close1"><span class="txt-theme"><i onclick="rmList(this,\''+numlabel+'\','+number+','+(itemSelected.length-1)+')" style="cursor:pointer; color:#e44444" class="fa fa-times" aria-hidden="true"></i></span></div></div>').appendTo('.contentList');
    }
...
$(".pad3num_list").html("");
  ...
  for(i=num;i<iMax;i++) {
    ...
    rs+='<li><a onclick="ListClick(this)" href="javascript:void(0)" data-numlabel="'+ans+'" data-numtext="3 ตัว" class="btn btn-listNum listNum'+ans+' '+active+'"><span>'+ans+'</span></a></li>';
  }
...
function chgBYbt(unitChg) {
  ...
  if((itemSelected.length*unitChg)-parseInt($("#headTop").html())>0){
    if(itemSelected.length>0){$("#itemSelectedNull").hide();$(".header").show(); $(".wbtn_cleardata").show()}else{$(".wbtn_cleardata").hide(); $(".header").hide();$("#itemSelectedNull").show();} //Show label & Header
...
for(ii=0;ii<itemSelected.length;ii++) {
    ...
    $('<div data-number="'+number+'"  class="deprive'+deprive+' ll'+id+' listSelected list'+number+'"><div class="content"><span class="txt-theme">'+number.substr(0,3)+'</span><br/><span class="label" style="font-size:15px">('+numtext+')</span></div><div style="font-size:15px !important;position : absolute !important;top:0px !important;right:5px !important;" class="close1"><span class="txt-theme"><i onclick="rmList(this,\''+numlabel+'\','+number+','+id+')" style="cursor:pointer; color:#e44444" class="fa fa-times" aria-hidden="true"></i></span></div></div>').appendTo('.contentList');
}
...
$("#btchageVal").click(function(){
  ...
  if((itemSelected.length*parseInt($("#txtChg").val()))-parseInt($("#headTop").html())>0){
    alert('เครดิตไม่พอ กรุณาเติมเงิน!');
    if(itemSelected.length>0){$("#itemSelectedNull").hide();$(".header").show(); $(".wbtn_cleardata").show()}else{$(".wbtn_cleardata").hide(); $(".header").hide();$("#itemSelectedNull").show();} //Show label & Header
    return;
  }
...
function save() {
  if($("#setname").val()==''){$("#setname").focus();return false;}
  $("#dataList").val(JSON.stringify(itemSelected));
  $("#nameSet").val($("#setname").val());
  //alert(JSON.stringify(itemSelected));
  //console.log(itemSelected);
  //location.reload();
  $.ajax({
    type: "POST",
    url: '<?php echo base_url('service/active/lottosetsave'); ?>',
    data: $("#formSave").serialize(), // serializes the form's elements.
    success: function(data)
    {
      alert(data.message);
      //console.log(data);
      if(data.status=='success') {
        window.location.replace('<?php echo site_url('lottonew/setNumber');?>/'+data.set_id);
      }else {
