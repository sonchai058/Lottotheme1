...
<h1 class="text-center txt-theme">งวดออกหวยล่วงหน้า</h1>
    ...
    <div class="row main_left" style="/*background-color:rgba(0,0,0,0.1);*/background-color:#450B08; border-radius: 10px; margin:0 auto; padding: 10px;border: 3px #f3b70c solid;">
      ...
      <div class="col-xs-6 col-sm-6 text-right" style="">
        <button  data-toggle="modal" data-target="#myModal0" onclick="return false"  class="btn btn-warning btn-trans" style="font-weight:bold">
            <i class="fa fa-plus-square-o" aria-hidden="true"></i> เพิ่มงวด
        </button>
                      <!-- Modal -->
                            ...
                            <div class="modal-body modal-p" style="">
                                <div class="row">
                                    <div class="col-xs-12 text-center" style="padding-left: 30px;padding-right: 30px;">
                                        <h2 style="text-align:center" class="font-weight-bold font-title text-h5 text-center">เพิ่มงวด</h2>
                                    </div>
                                </div>
                                <form id="formAdd" method="post">
                                    ...
                                    <div class="row">
                                        <div class="col-xs-12">
                                            <label>วันที่</label>
                                            <input type="date" class="form-control" name="date_closed" placeholder="วันที่" value="<?php //งวดที่มากกว่างวดล่าสุด ?>">
                                        </div>
                                    </div> 
                                    <br/>
                                    <div class="row">
        ...
        <div class="col-xs-12 col-sm-12" style="min-height:250px;/*background-color:rgba(0,0,0,0.1);*/background-color:#450B08; padding-bottom: 30px; margin-top:10px;">
                                ...
                                <div class="row" style="">
                                    <div class="col-xs-12">
                                            ...
                                            <thead><tr><th width="5%">ลำดับ</th><th>งวดวันที่</th><th>#</th></tr></thead>
                                            <tbody>
                                        <?php
                                            if(empty($dataList)){
                                        ?>
                                        ...
                                        ?>
                                                <tr><td><?php echo ($key+1);?></td><td><?php echo substr($data['date_closed'],8,2).'/'.substr($data['date_closed'],5,2).'/'.substr($data['date_closed'],0,4);?></td>
                                                <td width="20%">
                                                            ...
                                                            <div class="modal-body modal-p" style="">
                                                                ...
                                                                <form class="formEdit" id="formEdit<?php echo $data['id'];?>" method="post">
                                                                    ...
                                                                    <div class="row">
                                                                        <div class="col-xs-12">
                                                                            <label>วันที่</label>
                                                                            <input type="date" class="form-control" name="date_closed" placeholder="'วันที่'" value="<?php echo $data['date_closed'];?>" style="width:100%">
                                                                        </div>
                                                                    </div> 
                                                                    <br/> 
                                                                    <div class="row">
...
$("#bt_addSubmit").click(function(){
    $.ajax({
    type: "POST",
    url: '<?php echo base_url('lottonew/ajax_insertLottodate');?>',
...
function formEditSave(id){
    $.ajax({
    type: "POST",
    url: '<?php echo base_url('lottonew/ajax_updateLottodate');?>/'+id,
...
function delItem(id) {
    $.ajax({
    type: "POST",
    url: '<?php echo base_url('lottonew/ajax_deleteLottodate');?>/'+id,
    data: $("#formEdit"+id).serialize(), // serializes the form's elements.
    success: function(data)
    {
      alert(data.message);
