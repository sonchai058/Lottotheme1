...
function trans_all_to_niki() {

// alert('TEST register API ');
var submit_button = $("#trans_all_to_niki");

submit_button.html('<i class="fas fa-spinner fa-spin"></i> กำลังตรวจสอบข้อมูล...');
submit_button.attr('disabled','disabled');
$.ajax({
    type: 'POST',
    url: '<?php echo base_url('api/member/transfer/trans_all_to_niki'); ?>',
    dataType: 'json',
    success:function(data){
        // console.log(data);
        if (data.status == 'true') {
...
submit_button.attr('disabled','disabled');
    $.ajax({
    type: 'GET',
    url: '<?php echo base_url('api/member/playgame/playniki/'); ?>'+game_code,
    dataType: 'json',
    success:function(data){
        // alert(data);
        if (data != '') { 
